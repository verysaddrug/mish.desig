-- Параметризованная версия запроса рейтинга пользователя
-- Запускать с параметром: psql -v username="'user123'" -f user_rating_param.sql
...
user_info AS (
    ...
    FROM 
        users
    WHERE 
        username = :username -- Параметр, передаваемый через командную строку
),
